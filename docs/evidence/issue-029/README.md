...
| Focused upload suites | 72 unit/contract/security/PostgreSQL cases passed, including real-current-PR issuance, head-update races, strict archive EOF, direct-INSERT lifecycle rejection, recovery/finalizer and decommission/finalizer races, exact repository/scope recovery, retention-renewal serialization, bounded decommission terminality/idempotent retry, cleanup retry, live storage, and immutable blob linking. |
...
| Broad/coverage | The broad run passed 863 tests with one expected live-MCP-profile skip and two browser-marker deselections; the only failure was the unrelated canvas `<1.0s` wall-clock assertion under coverage instrumentation. That performance gate passed unchanged 3/3 without coverage. The coverage lane then excluded only that exact timing test and passed 863 tests, one expected skip, and three deselections in 402.18 seconds. Total line/branch coverage met the 85% gate: 17,078 statements, 1,984 missed, 4,780 branches, and 1,078 partial branches. |
