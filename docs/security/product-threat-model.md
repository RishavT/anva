...
| Supply-chain artifact tampering | Clean source, exact OCI revision, rebuilt skills, scans, SBOM, immutable version-and-digest references, signed provenance | Local wheel/skills/SBOM/scan artifacts exist. Rishav Thakker approved the exact 13-CVE/16-tuple no-fix baseline for v0.1.0 from 2026-08-26 through 2026-09-25. The checked-in decision is not the release acceptance: publication must freshly match the exact tuple set and generate source-, scan-, version-, and immutable-digest-bound acceptance. Source vulnerability/secret/misconfiguration gating excludes operator-owned directories from distributable output. No registry digest, tag, signature/provenance, or publication is claimed. |
