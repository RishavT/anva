...
| Supply-chain artifact tampering | Clean source, exact OCI revision, rebuilt skills, scans, SBOM, immutable version-and-digest references, signed provenance | Local wheel/skills/SBOM/scan artifacts exist. The image gate uses 14 reviewed no-vendor-fix exceptions expiring 2026-08-18; source vulnerability/secret/misconfiguration gating excludes operator-owned directories from distributable output. No final exact-commit checksum manifest, registry digest, tag, signature/provenance, or publication is claimed. This work does not add, enable, or change a GitHub Actions workflow. |
