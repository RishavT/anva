...
| Recovery or decommission races finalization | Recovery is exact-repository/scope authorized and atomically leases `RECOVERING` before storage cleanup; decommission claims ISSUED/RECEIVING rows before inactivation, processes bounded batches including `DELETE_PENDING`, and cannot complete until an organization-locked proof finds only `DELETED` blobs and terminal uploads; finalization locks/rechecks the active organization first | Cross-repository recovery, recovery/finalizer, decommission/finalizer, candidate-bound, pending-failure, and idempotent-retry tests |
