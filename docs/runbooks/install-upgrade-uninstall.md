...
This runbook covers the GitHub-native `v0.1.1` installation path and the
...
## GitHub-native v0.1.1 installation
...
ANVA_RELEASE_IMAGE="$(jq -er \
  ...
  anva-v0.1.1/release-manifest.json)"
...
docker tag "${ANVA_RELEASE_IMAGE}" ghcr.io/rishavt/anva:0.1.1
```
...
```sh
mkdir anva-install
tar -xzf anva-v0.1.1/anva-install-0.1.1.tar.gz -C anva-install
cd anva-install/anva-0.1.1
cp .env.example .env
