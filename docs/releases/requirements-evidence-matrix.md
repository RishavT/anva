...
| Issue 13; V3 20.8, 28.16; SEC-006–008 | Retention and deletion | candidate | Exact-source tests cover server-owned time, explicit expiry plus organization minimum, tenant-only bucket cleanup, recent human/CSRF/two-confirmation decommission, and bearer/CLI rejection. No post-setup reauthentication exists, so decommission is unavailable after 15 minutes; physical deletion is absent | Reauthentication flow, interruption/recovery and retained/deleted-data operational acceptance; no erasure overclaim |
