# GitHub-native v0.1.1 release

The release workflow publishes only an existing exact `v0.1.1` tag. It is a
...
2. Separately create the protected `v0.1.1` tag at that exact commit. Never move,
...
gh workflow run release.yml --repo rishavt/anva --ref main \
  -f tag=v0.1.1 -f source_commit="$ANVA_SOURCE_COMMIT" \
...
```sh
gh release download v0.1.1 --repo rishavt/anva --dir anva-v0.1.1
(cd anva-v0.1.1 && sha256sum --check SHA256SUMS)
ANVA_SOURCE_COMMIT="$(jq -er '.source_commit' anva-v0.1.1/release-manifest.json)"
ANVA_SOURCE_TAG=v0.1.1
ANVA_SOURCE_VERSION=0.1.1
ANVA_SOURCE_PREDICATE_TYPE=https://github.com/RishavT/anva/attestations/source/v1
for artifact in anva-v0.1.1/*; do
...
for the published v0.1.0 history and must not be used for v0.1.1.
