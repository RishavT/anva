# GitHub-native v0.1.4 release

The release workflow publishes only an existing exact `v0.1.4` tag. It is a
...
2. Separately create the protected `v0.1.4` tag at that exact commit. Never move,
...
gh workflow run release.yml --repo rishavt/anva --ref main \
  -f tag=v0.1.4 -f source_commit="$ANVA_SOURCE_COMMIT" \
...
for the published v0.1.0 history and must not be used for v0.1.4.
