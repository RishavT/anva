...
def test_manual_evaluator_claim_and_submit_http_adapters(client: Client) -> None:
    ...
    completed_claim = SimpleNamespace(
        task=SimpleNamespace(
            id=task_id,
            assurance_run_id=run_id,
            assurance_run=completion.run,
            claimant="fresh-agent",
            claimed_by_actor_type="SERVICE",
            claimed_by_actor_id="review-service",
            claimed_by_credential_id=claim.task.claimed_by_credential_id,
            result_artifact=SimpleNamespace(content_hash="2" * 64),
        ),
    ...
    with (
        ...
        patch(
            ...
            side_effect=[None, claim, human_claim, completed_claim],
        ) as claim_task,
        patch(
    ...
    assert completed_claimed.status_code == 200
    assert completed_claimed.json()["status"] == "COMPLETED"
    assert completed_claimed.json()["result_hash"] == "2" * 64
    assert "claim_token" not in completed_claimed.json()
    assert "request" not in completed_claimed.json()
    ...
    validate_acceptance_http_response("claimManualEvaluatorTask", 200, completed_claimed.json())
