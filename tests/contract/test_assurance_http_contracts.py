...
from anva.core.models import AssuranceRun, Finding
...
def test_manual_evaluator_claim_and_submit_http_adapters(client: Client) -> None:
    ...
    completion = SimpleNamespace(
        run=SimpleNamespace(id=run_id, state=AssuranceRun.State.COMPLETED),
    ...
    with (
        patch("anva.core.views._actor", return_value=object()),
        patch("anva.core.views.claim_evaluator_task", side_effect=[None, claim]) as claim_task,
        patch(
        ...
        claimed = client.post(
            claim_url,
            data=json.dumps({"claimant": "fresh-agent", "lease_seconds": 30}),
            content_type="application/json",
        )
