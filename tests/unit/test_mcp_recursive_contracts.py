...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Bearer token, which is prohibited, was replaced.",
        "Bearer credentials, which were deprecated, are documented controls.",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Bearer token, actual-secret-value",
        "Bearer token, which is hunter2",
        "Bearer token, which equals actual-secret-value",
        "Bearer token, and hunter2",
        ...
        *[
            f"Bearer token {connector} {prefix}{value}"
        ...
        *[
            f"Bearer token {modifier} {assignment}{value}"
            for modifier in ("authentication", "only")
            for assignment in ("is ", "equals ", ": ", "= ")
            for value in ("hunter2", "actual-secret-value", "пароль")
        ],
