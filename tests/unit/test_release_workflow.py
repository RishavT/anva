...
def test_dispatch_uses_main_workflow_but_binds_products_to_the_tag_commit() -> None:
    ...
    assert environment["ANVA_VERSION"] == "0.1.7"
    ...
    assert inputs["tag"]["default"] == "v0.1.7"
...
if [ "$1" = ls-remote ]; then
  case "$TAG_RESPONSE" in
    nonzero_partial)
      printf '%s\\trefs/tags/v0.1.7\\n' "$SOURCE_COMMIT"
      exit 2
      ;;
    missing) exit 2 ;;
    malformed) printf '%s\\trefs/tags/v0.1.7\\n' not-a-commit ;;
    duplicate_direct)
      printf '%s\\trefs/tags/v0.1.7\\n' "$SOURCE_COMMIT"
      printf '%s\\trefs/tags/v0.1.7\\n' "$SOURCE_COMMIT"
      ;;
    duplicate_peeled)
      ...
      printf '%s\\trefs/tags/v0.1.7^{}\\n' "$SOURCE_COMMIT"
      ;;
    valid) printf '%s\\trefs/tags/v0.1.7\\n' "$SOURCE_COMMIT" ;;
  esac
