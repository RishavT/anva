...
def _write_protected_decision(path: Path, report: Path) -> Path:
    ...
    decision = {
        ...
        "workflow_ref": "RishavT/anva/.github/workflows/release.yml@refs/tags/v0.1.4",
...
def _write_release_artifacts(directory: Path) -> None:
    for name in (
        "anva-0.1.4-py3-none-any.whl",
        "anva-install-0.1.4.tar.gz",
...
def _write_v014_approval_fixture(path: Path) -> Path:
    payload = json.loads(
        Path("docs/security/vulnerability-exceptions.json").read_text(encoding="utf-8")
    )
    payload["release_version"] = "0.1.4"
...
def test_release_verifier_rejects_stale_candidate_and_tampering(tmp_path: Path) -> None:
    ...
    (tmp_path / "anva-0.1.4-py3-none-any.whl").write_bytes(b"tampered")
...
def test_release_verifier_rejects_traversal_symlink_and_unrecorded_artifact(
    ...
    artifact = tmp_path / "anva-0.1.4-py3-none-any.whl"
    artifact.unlink()
...
def test_release_manifest_rejects_missing_scan_artifact(tmp_path: Path) -> None:
    (tmp_path / "anva-0.1.4-py3-none-any.whl").write_bytes(b"wheel")
...
@pytest.mark.unit
def test_checked_in_v010_approval_is_rejected_for_v014(tmp_path: Path) -> None:
...
@pytest.mark.unit
def test_no_checked_in_v014_risk_approval_exists() -> None:
    assert not Path("docs/security/vulnerability-exceptions-v0.1.4.json").exists()
...
def test_risk_acceptance_binds_exact_candidate_and_preserves_tuple_evidence(
    ...
    approval = _write_v014_approval_fixture(tmp_path / "approval.json")
...
@pytest.mark.parametrize(
    ...
    [
        (f"ghcr.io/rishavt/anva@sha256:{'e' * 64}", APPROVED_DIGEST, "0.1.4", "reference"),
        (f"ghcr.io/rishavt/anva@{APPROVED_DIGEST}", "sha256:short", "0.1.4", "digest"),
...
def test_risk_acceptance_rejects_candidate_identity_replay(
    ...
    with pytest.raises(ValueError, match=message):
        build_vulnerability_risk_acceptance(
            input_path=_write_v014_approval_fixture(tmp_path / "approval.json"),
            vulnerability_report_path=Path(
                "tests/fixtures/release/vulnerability-exceptions-sanitized-trivy.json"
            ),
