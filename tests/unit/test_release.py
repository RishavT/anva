...
def _write_protected_decision(path: Path, report: Path) -> Path:
    ...
    decision = {
        ...
        "workflow_ref": "RishavT/anva/.github/workflows/release.yml@refs/tags/v0.1.7",
...
def _write_release_artifacts(directory: Path) -> None:
    for name in (
        "anva-0.1.7-py3-none-any.whl",
        "anva-install-0.1.7.tar.gz",
...
def test_release_verifier_rejects_stale_candidate_and_tampering(tmp_path: Path) -> None:
    ...
    (tmp_path / "anva-0.1.7-py3-none-any.whl").write_bytes(b"tampered")
...
def test_release_verifier_rejects_traversal_symlink_and_unrecorded_artifact(
    ...
    artifact = tmp_path / "anva-0.1.7-py3-none-any.whl"
    artifact.unlink()
...
def test_release_manifest_rejects_missing_scan_artifact(tmp_path: Path) -> None:
    (tmp_path / "anva-0.1.7-py3-none-any.whl").write_bytes(b"wheel")
...
def test_no_checked_in_v015_risk_approval_exists() -> None:
    assert not Path("docs/security/vulnerability-exceptions-v0.1.7.json").exists()
...
@pytest.mark.parametrize(
    ...
    [
        (f"ghcr.io/rishavt/anva@sha256:{'e' * 64}", APPROVED_DIGEST, "0.1.7", "reference"),
        (f"ghcr.io/rishavt/anva@{APPROVED_DIGEST}", "sha256:short", "0.1.7", "digest"),
