...
from anva.release import (
    ...
    build_trivy_ignorefile_from_decision,
...
def _write_protected_decision(path: Path, report: Path) -> Path:
    ...
    _write_trivy_report(report, [finding])
    decision = {
        "schema_version": 1,
        ...
        "workflow_ref": "RishavT/anva/.github/workflows/release.yml@refs/tags/v0.1.5",
        "environment_approval_record_sha256": "a" * 64,
        "proposal_sha256": "b" * 64,
        "proposal_report_sha256": hashlib.sha256(report.read_bytes()).hexdigest(),
        "proposal_database_metadata_sha256": "d" * 64,
        "proposal_source_security_report_sha256": "e" * 64,
        "proposal_source_scan_diagnostic_sha256": "f" * 64,
...
def _write_release_artifacts(directory: Path) -> None:
    for name in (
        "anva-0.1.5-py3-none-any.whl",
        "anva-install-0.1.5.tar.gz",
...
def _write_v015_approval_fixture(path: Path) -> Path:
    payload = json.loads(
        Path("docs/security/vulnerability-exceptions.json").read_text(encoding="utf-8")
    )
    payload["release_version"] = "0.1.5"
...
def test_release_verifier_rejects_stale_candidate_and_tampering(tmp_path: Path) -> None:
    ...
    (tmp_path / "anva-0.1.5-py3-none-any.whl").write_bytes(b"tampered")
...
def test_release_verifier_rejects_traversal_symlink_and_unrecorded_artifact(
    ...
    artifact = tmp_path / "anva-0.1.5-py3-none-any.whl"
    artifact.unlink()
...
def test_release_manifest_rejects_missing_scan_artifact(tmp_path: Path) -> None:
    (tmp_path / "anva-0.1.5-py3-none-any.whl").write_bytes(b"wheel")
...
@pytest.mark.unit
def test_checked_in_v010_approval_is_rejected_for_v015(tmp_path: Path) -> None:
...
@pytest.mark.unit
def test_no_checked_in_v015_risk_approval_exists() -> None:
    assert not Path("docs/security/vulnerability-exceptions-v0.1.5.json").exists()
    assert not Path("release/vulnerability-risk-acceptance.json").exists()


@pytest.mark.unit
def test_protected_decision_gate_binds_exact_candidate_and_report(tmp_path: Path) -> None:
    report = tmp_path / "report.json"
    decision = _write_protected_decision(tmp_path / "decision.json", report)
    output = tmp_path / ".trivyignore"
    assert build_trivy_ignorefile_from_decision(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("workflow_ref", "RishavT/anva/.github/workflows/release.yml@refs/tags/stale"),
        ("image_reference", f"ghcr.io/rishavt/anva@sha256:{'5' * 64}"),
    ],
)
def test_protected_decision_rejects_stale_expected_identity(
...
def test_risk_acceptance_binds_exact_candidate_and_preserves_tuple_evidence(
    ...
    approval = _write_v015_approval_fixture(tmp_path / "approval.json")
...
@pytest.mark.parametrize(
    ...
    [
        (f"ghcr.io/rishavt/anva@sha256:{'e' * 64}", APPROVED_DIGEST, "0.1.5", "reference"),
        (f"ghcr.io/rishavt/anva@{APPROVED_DIGEST}", "sha256:short", "0.1.5", "digest"),
...
def test_risk_acceptance_rejects_candidate_identity_replay(
    ...
    with pytest.raises(ValueError, match=message):
        build_vulnerability_risk_acceptance(
            input_path=_write_v015_approval_fixture(tmp_path / "approval.json"),
            vulnerability_report_path=Path(
                "tests/fixtures/release/vulnerability-exceptions-sanitized-trivy.json"
            ),
